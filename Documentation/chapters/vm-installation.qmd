...
- Install Shiny Server from the `.deb` package.
- Configure Apache vhost, LDAP, and systemd environment.
...
### 6) Configure LDAP mode (systemd override)
...
### 7) Configure Apache (test HTTPS redirect)
...
Leave the current `ngs-vm_01-main.conf` active for the Perl system.

### 2) Add a parallel Shiny vhost on `:8443` (or `:1234`)
