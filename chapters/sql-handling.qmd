...
```bash
sudo -u shiny sqlite3 /srv/shiny-server/sequencing-app/sequencing_projects.db \
"INSERT OR IGNORE INTO users (username, is_admin) VALUES ('ldapadmin', 1);"
```
...
```bash
sudo -u shiny sqlite3 /srv/shiny-server/sequencing-app/sequencing_projects.db \
"INSERT OR IGNORE INTO users (username, password, email, is_admin) VALUES ('ldapadmin', 'ldap-only', 'ldap.admin@example.org', 1);"
```
...
```bash
sudo -u shiny sqlite3 /srv/shiny-server/sequencing-app/sequencing_projects.db \
"UPDATE users SET is_admin=1 WHERE username='ldapadmin';"
```
...
```bash
scp /www/mpiseq2/exports/legacy_projects.csv USER@shiny-vm:/home/exampleuser/example-repo/exports/
```
...
sudo -u shiny python3 /home/exampleuser/example-repo/scripts/import_legacy_projects.py \
  ...
  --csv /home/exampleuser/example-repo/exports/legacy_projects.csv
sudo systemctl start shiny-server
```
...
1. Manual **pre-deploy rollback backups** (recommended before any deploy/maintenance): copy the app folder, the SQLite DB, and the Apache vhost config with a timestamp. See **Deploy & Operations**.
...
3. **Automatic bi-weekly backups**: enabled in the app. It creates one consistent SQLite snapshot per two-week ISO calendar period when the app has an active session.

The default backup directory is `/srv/shiny-server/DB_backup`. Set `DB_BACKUP_DIR` in the Shiny service environment to override it. Automatic backup creation is recorded in `backup_logs`.

For example, to inspect an explicitly configured VM backup directory:

```bash
sudo -u shiny ls -lh /path/from/DB_BACKUP_DIR
```
...
**Update sample service type costs**
