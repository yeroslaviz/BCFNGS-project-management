...
- [SQL (moved)](#sql-moved)
- [Auth mode switching](#auth-mode-switching)
- [Apache proxy](#apache-proxy)
- [LDAP Deploy Troubleshooting (by symptom)](#ldap-deploy-troubleshooting-by-symptom)
...
```bash
cd /Users/exampleuser/Documents/Github/example-repo/_site
...
```bash
quarto preview /Users/exampleuser/Documents/Github/example-repo
```
...
```
AuthLDAPURL "ldaps://ldap01.example.internal:636/dc=biochem,dc=mpg,dc=de?uid?sub?(objectClass=posixAccount)"
```
...
```
nc -vz ldap01.example.internal 636
```

2. Test TLS bypass:

```
LDAPTLS_REQCERT=never ldapsearch -x -H ldaps://ldap01.example.internal:636 \
...
### `ERR_SSL_PROTOCOL_ERROR` for `https://bcf-vm/`
...
1.  On your **local machine**, map VM IP to hostnames:

```
sudo sh -c 'echo "192.0.2.55 bcf-vm bcf-vm.biochem.mpg.de" >> /etc/hosts'
sudo dscacheutil -flushcache
sudo killall -HUP mDNSResponder
```

2.  On `bcf-vm`, keep only the intended site active and enable SSL:
...
```
curl -kI -H 'Host: bcf-vm.biochem.mpg.de' https://127.0.0.10/
```
...
### LDAP mode configured, but app still shows local page
...
### `LDAP DEBUG ERROR (onFlushed): Operation not allowed without an active reactive context`
...
### Main URL still serves Perl after cutover
...
### Perl fallback on `:9443` returns connection refused
...
### LDAP login works on `:8443` but fails on no-port `:443`
...
```bash
sudo cp /home/exampleuser/example-repo/scripts/ngs-vm-shiny-443.conf /etc/apache2/sites-available/ngs-vm-shiny-443.conf
sudo a2ensite ngs-vm-shiny-443.conf
sudo apache2ctl -t
sudo systemctl restart apache2
```
...
All SQL database commands and explanations are documented in the **SQL Handling** chapter (`chapters/sql-handling.qmd`).  
...
# LDAP
ldapsearch -x -H ldaps://ldap01.example.internal:636 \
  -b "dc=biochem,dc=mpg,dc=de" "(uid=USER)" mail
