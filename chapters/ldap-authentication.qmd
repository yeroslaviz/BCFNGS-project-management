...
- [Dependencies (Ubuntu + R)](#dependencies-ubuntu--r)
...
```{bash}
ldapsearch -x -H ldaps://ldap01.example.internal -b "dc=biochem,dc=mpg,dc=de" "(uid=YOURUSER)"
```
...
1. Update Apache config (template in `mpiseq2/ngs-vm_01-main.conf`).
2. Enable modules:
   ```bash
   sudo a2enmod ldap authnz_ldap headers
   sudo systemctl reload apache2
