...
```bash
bash /Users/exampleuser/Documents/Github/example-repo/scripts/check_requirements.sh
```

Install missing apt packages:

```bash
bash /Users/exampleuser/Documents/Github/example-repo/scripts/check_requirements.sh --install
```

Install missing R packages (including `rJava`):

```bash
bash /Users/exampleuser/Documents/Github/example-repo/scripts/check_requirements.sh --install-r
```
...
URI             ldaps://ldap01.example.internal ldaps://ldap02.example.internal ldaps://ldap03.example.internal
TLS_REQCERT     hard
...
```bash
ldapsearch -x -H ldaps://ldap01.example.internal:636 -b "dc=biochem,dc=mpg,dc=de" "(uid=<user>)" mail
...
```bash
curl -kI -H 'Host: ngs-vm.biochem.mpg.de' https://127.0.0.10:8443/
curl -kI -u USER -H 'Host: ngs-vm.biochem.mpg.de' https://127.0.0.10:8443/sequencing-app/
```
...
```bash
psql -U legacyadmin -h 127.0.0.10 -d mpiseq2_db -c "\
\\copy (\
  SELECT \
    p.id AS legacy_id, \
...
```bash
# run on bcf-vm once
mkdir -p /home/exampleuser/example-repo/exports/

# run on ngs-vm (source)
scp /www/mpiseq2/exports/legacy_projects.utf8.csv \
  exampleuser@bcf-vm:/home/exampleuser/example-repo/exports/
```

If importing into Shiny on `ngs-vm` (same host as perl system):

```bash
cp -R /www/mpiseq2/exports/ /home/exampleuser/example-repo/
```
...
If `/home/exampleuser` is not traversable by `shiny` (permission denied), either:

- quick fix:

```bash
sudo chmod o+x /home/exampleuser
```
...
```bash
sudo -u shiny python3 /home/exampleuser/example-repo/scripts/import_legacy_projects.py \
  --db /srv/shiny-server/sequencing-app/sequencing_projects.db \
  --csv /home/exampleuser/example-repo/exports/legacy_projects.utf8.csv
...
- Detailed SQL notes remain in `chapters/sql-handling.qmd`.
